...
export class DataCollector {
    ...
    if (this.outputChannel) {
      ...
      if (this.vscodeEdition === VSCodeEdition.REMOTE_SSH) {
        ...
        this.outputChannel.appendLine(
          `VSCODE_AGENT_FOLDER: ${process.env['VSCODE_AGENT_FOLDER'] || '未设置'}`,
        );
        this.outputChannel.appendLine(
          `VSCODE_SSH_HOST: ${process.env['VSCODE_SSH_HOST'] || '未设置'}`,
        );
        this.outputChannel.appendLine(
          `REMOTE_SSH_EXTENSION: ${process.env['REMOTE_SSH_EXTENSION'] || '未设置'}`,
        );
        this.outputChannel.appendLine(
  ...
  async getExtensions(): Promise<ExtensionsExport> {
    ...
    if (userSettings && typeof userSettings === 'object' && userSettings['extensions.disabled']) {
      disabledList = Array.isArray(userSettings['extensions.disabled'])
        ? userSettings['extensions.disabled']
        : [];
    }
            ...
            for (const file of files) {
              if (file.endsWith('.code-snippets') || file.endsWith('.json')) {
                ...
                try {
                  ...
                  if (this.outputChannel) {
                    this.outputChannel.appendLine(
                      `Failed to read workspace snippet file ${file}: ${error}`,
                    );
                  }
                }
              }
